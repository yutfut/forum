...
create unique index if not exists idx_user_by_nickname on user using hash (nickname);   --    unique  using hash
...
create unique index if not exists idxex_forum_by_slug on forum using hash (slug);   --    unique  using hash
