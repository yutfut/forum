...
create unique index if not exists index_post_by_thread on post using hash (thread);
...
create unique index if not exists index_thread_by_forum on thread using hash (forum);
