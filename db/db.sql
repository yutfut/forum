...
create index if not exists index_thread_by_id_slug on thread using btree (slug, id);
...
create index if not exists index_forum_by_slug on forum using hash (slug);   --    unique  using hash
...
create unique index if not exists index_post_by_thread_path on post using btree (thread, path);
...
create index if not exists index_post_by_thread on post using btree (thread);
...
create index if not exists index_post_by_thread on forum_user using btree (user, forum);

-- `select "id", "title", "author", "forum", "message", "votes", "slug", "created"
-- from "thread"
-- where "forum" = $1`
drop index if exists index_thread_by_forum;
create index if not exists index_post_by_thread on thread using btree (forum);